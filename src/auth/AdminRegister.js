...
const styles = `
  ...
  .pal-token-note {
    background: #fdf8f0; border-left: 2px solid rgba(200,150,60,0.5);
    padding: 10px 14px; margin-top: 6px;
    font-family: 'Oswald', sans-serif; font-size: 0.6rem; letter-spacing: 0.1em;
    color: #9a7a3a; line-height: 1.7;
  }
  .pal-btn-primary {
  ...
  .pal-success {
    background: #fdf8f0; border-left: 2px solid #c8963c;
    font-family: 'Oswald', sans-serif; font-size: 0.72rem; letter-spacing: 0.1em;
    color: #7a5a1a; padding: 10px 14px; margin-bottom: 16px;
  }
  .pal-closed-box {
    border: 1px solid #e2d9cc; background: #faf9f6;
    padding: 20px; margin-bottom: 20px; text-align: center;
  }
  .pal-closed-text {
    font-family: 'Libre Baskerville', serif; font-size: 0.85rem;
    color: #9a8060; line-height: 1.7;
  }
  .pal-btn-go {
    display: block; width: 100%; background: #d4520a; border: 1px solid #8c3206; color: #fff;
    font-family: 'Oswald', sans-serif; font-size: 0.72rem; letter-spacing: 0.26em;
    text-transform: uppercase; padding: 11px 20px; cursor: pointer;
    text-align: center; text-decoration: none; transition: background 0.15s; box-sizing: border-box;
  }
  .pal-btn-go:hover { background: #f07228; }
  .pal-link-row {
...
export default function AdminRegister() {
  const [formData, setFormData] = useState({ name: '', email: '', password: '', passwordConfirm: '', registrationToken: '' });
  ...
  useEffect(() => {
    const checkAdmin = async () => {
      try {
        const response = await axios.get(`${API_URL}/api/auth/test-admin`);
        if (response.data.email) setAdminExists(true);
      } catch (err) {
  ...
  const { name, email, password, passwordConfirm, registrationToken } = formData;
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    setError('');
    if (password !== passwordConfirm) { setError('Passwords do not match'); setIsLoading(false); return; }
    if (!registrationToken) { setError('Registration token is required'); setIsLoading(false); return; }
    try {
      const response = await axios.post(`${API_URL}/api/auth/register`, { name, email, password, registrationToken });
    ...
    <>
      ...
      <div className="pal-auth-root">
        ...
        <div className="pal-status-tag">
          ...
          {adminExists ? 'Registration Closed' : 'One-Time Setup'}
        </div>
        ...
        <div className="pal-card">
          ...
          <div className="pal-card-inner">
            ...
            <h1 className="pal-title">{adminExists ? 'Access Restricted' : 'Admin Setup'}</h1>
            <p className="pal-subtitle">{adminExists ? 'Registration Closed' : 'Secure Registration'}</p>
            <div className="pal-divider" />

            {adminExists ? (
              <>
                <div className="pal-closed-box">
                  <p className="pal-closed-text">An admin account already exists.<br />Registration is no longer available.</p>
                </div>
                <Link to="/login" className="pal-btn-go">⬡ Go to Login</Link>
              </>
            ) : success ? (
              <div className="pal-success">◈ {success}</div>
            ) : (
              <form onSubmit={handleSubmit}>
                <div className="pal-field">
                  <label className="pal-label" htmlFor="name">Full Name</label>
                  <input id="name" name="name" className="pal-input" type="text" required value={name} onChange={onChange} placeholder="Administrator name" />
                </div>
                <div className="pal-field">
                  <label className="pal-label" htmlFor="email">Email Address</label>
                  <input id="email" name="email" className="pal-input" type="email" required value={email} onChange={onChange} placeholder="admin@example.com" />
                </div>
                <div className="pal-field">
                  <label className="pal-label" htmlFor="password">Password</label>
                  <input id="password" name="password" className="pal-input" type="password" required value={password} onChange={onChange} placeholder="••••••••" />
                </div>
                <div className="pal-field">
                ...
                <div className="pal-field">
                  <label className="pal-label" htmlFor="registrationToken">Registration Token</label>
                  <input id="registrationToken" name="registrationToken" className="pal-input" type="password" required value={registrationToken} onChange={onChange} placeholder="••••••••••••" />
                  <div className="pal-token-note">A secure token is required. Only authorised personnel have access to this token.</div>
                </div>
            ...
            {!adminExists && !success && (
              <div className="pal-link-row">
                <span className="pal-link-text">Already have an account? <Link to="/login" className="pal-link-accent">Sign in →</Link></span>
              </div>
            )}
          </div>
        </div>
